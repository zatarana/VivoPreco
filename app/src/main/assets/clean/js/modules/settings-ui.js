window.SettingsUI=(function(){
  function render(data){
    ...
    Dom.render(`${Components.card('Resumo dos dados',`Carteiras: <b>${s.wallets}</b><br>Transações: <b>${s.transactions}</b><br>Contas: <b>${s.bills}</b><br>Dívidas: <b>${s.debts}</b><br>Tarefas: <b>${s.tasks}</b><br>Projetos: <b>${s.projects}</b><br>Sessões de tempo: <b>${s.timeLogs}</b><br>Categorias: <b>${s.categories}</b><br>Orçamentos: <b>${s.budgets}</b><br>Metas: <b>${s.goals}</b><br>Cartões: <b>${s.cards}</b>`)}${Components.card('Dados locais',`Versão dos dados: <b>${data.version||1}</b><br>Migração realizada: <b>${data.migratedFromLegacy?'Sim':'Não'}</b><br>Dados antigos detectados: <b>${legacy?'Sim':'Não'}</b><br>Uso local aproximado: <b>${size} caracteres</b>`)}<div class="row">${Components.button('Exportar backup','exportBackup')}${Components.button('Importar backup','importBackup','secondary')}</div><div class="row">${Components.button('Carteiras','showWallets','secondary')}${Components.button('Planejamento','showPlanning','secondary')}</div><div class="row">${Components.button('Criar demonstração','seedDemo','secondary')}</div>${Components.card('Sobre','VivoPreco usa armazenamento local no aparelho. Exporte backup antes de limpar dados ou trocar de instalação.')} ${Components.button('Rodar migração novamente','runMigration','secondary')}${Components.button('Resetar dados','resetClean','danger')}<input id="backupFileInput" type="file" accept="application/json,.json" hidden>`);
  }
  ...
  function seedDemo(){
    StorageService.update(d=>{
      ...
      const task=TaskEngine.addTask(d,{title:'Conferir contas do mês',projectId:'project_finance',dueDate:Models.today(),priority:'P1',labels:['finanças']});
      ...
      PlanningEngine.addCard(d,{name:'Cartão principal',limit:1500,closingDay:5,dueDay:12,walletId:FinanceEngine.defaultWalletId(d)});
    });
