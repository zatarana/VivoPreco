window.App=(function(){
  ...
  function handleAction(action,id){
    try{
      ...
      if(action==='newProjectTask')return TasksUI.newTaskForm(id);
      ...
      if(action==='reopenTask')return TasksUI.reopenTask(id);
      if(action==='editTask')return TasksUI.editTaskForm(id);
      if(action&&action.startsWith('saveEditTask:'))return TasksUI.saveEditTask(action.split(':')[1]);
      if(action==='deleteTask')return TasksUI.deleteTask(id);
      if(action==='newSubtask')return TasksUI.newSubtaskForm(id);
      if(action&&action.startsWith('saveSubtask:'))return TasksUI.saveSubtask(action.split(':')[1]);
      if(action==='newComment')return TasksUI.newCommentForm(id);
      if(action&&action.startsWith('saveComment:'))return TasksUI.saveComment(action.split(':')[1]);
      if(action==='showUpcoming')return TasksUI.showUpcoming(data());
      if(action==='showTaskFilters')return TasksUI.showTaskFilters(data());
      if(action==='showLabels')return TasksUI.showLabels(data());
      if(action==='openLabel')return TasksUI.openLabel(id);
      if(action==='openProject')return TasksUI.openProject(id);
      if(action==='showTaskBoard')return TasksUI.showTaskBoard(data());
      if(action==='runMigration')return SettingsUI.runMigration();
      if(action==='resetClean')return SettingsUI.resetClean();
      if(action==='seedDemo')return SettingsUI.seedDemo();
