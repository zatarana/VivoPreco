window.StorageService=(function(){
  ...
  function seed(){return {version:2,preferences:{defaultWalletId:'wallet_main',demoCleaned:true},wallets:[Models.wallet({type:'corrente',realBalance:0,name:'Conta principal'})],transactions:[],bills:[],debts:[],categories:[{id:'cat_food',name:'Alimentação',type:'despesa'},{id:'cat_home',name:'Moradia',type:'despesa'},{id:'cat_debt',name:'Dívidas',type:'despesa'},{id:'cat_income',name:'Receita',type:'receita'},{id:'cat_general',name:'Geral',type:'ambos'},{id:'cat_card',name:'Cartão',type:'despesa'}],budgets:[],goals:[],cards:[],cardPurchases:[],timeLogs:[],activeTimer:null,projects:[{id:'project_inbox',name:'Inbox',color:'#2563eb',sections:['Entrada'],view:'list',kind:'comum',weeklyTargetMinutes:0},{id:'project_personal',name:'Pessoal',color:'#16a34a',sections:['Geral','Rotina'],view:'list',kind:'comum',weeklyTargetMinutes:0},{id:'project_finance',name:'Finanças',color:'#d97706',sections:['A Fazer','Em andamento','Concluído'],view:'board',kind:'comum',weeklyTargetMinutes:0},{id:'project_study',name:'Estudos',color:'#7c3aed',sections:['Matérias','Questões','Revisão'],view:'list',kind:'estudos',weeklyTargetMinutes:1200}],tasks:[]};}
  function isDemoInitialTx(t){return t&&t.description==='Entrada inicial'&&t.type==='receita'&&Number(t.value||0)===1500&&t.category==='Receita'&&!t.billId&&!t.debtId;}
  function normalize(data){
    ...
    data.wallets=data.wallets.map(w=>{const real=w.realBalance!==undefined?Number(w.realBalance||0):Number(w.initialBalance||0);return Object.assign({},w,{type:normalizeAccountType(w.type),realBalance:real,initialBalance:real});});
    data.preferences=data.preferences||{};
    if(!data.preferences.demoCleaned){
      data.transactions=(Array.isArray(data.transactions)?data.transactions:[]).filter(t=>!isDemoInitialTx(t));
      data.tasks=(Array.isArray(data.tasks)?data.tasks:[]).filter(t=>!['Revisar orçamento do mês','Estudar Direito Constitucional'].includes(t.title));
      data.preferences.demoCleaned=true;
    }
    ...
    data.cardPurchases=data.cardPurchases.map(p=>Object.assign({installments:1,installmentNumber:1,totalValue:Number(p.value||0),billId:null,invoiceMonth:String(p.date||Models.today()).slice(0,7)},p));
