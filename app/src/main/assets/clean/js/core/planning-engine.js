window.PlanningEngine=(function(){
  ...
  function invoiceTotal(data,cardId,month){const purchases=purchasesForInvoice(data,cardId,month);return r(purchases.reduce((s,p)=>s+n(p.value),0));}
  ...
  function closeCardInvoice(data,cardId,month,dueDate){ensure(data);const card=cardById(data,cardId);if(!card)throw new Error('Cartão não encontrado.');const purchases=(data.cardPurchases||[]).filter(p=>p.cardId===cardId&&p.invoiceMonth===month&&!p.billId);if(!purchases.length)throw new Error('Não há compras abertas para esta fatura.');const total=r(purchases.reduce((s,p)=>s+n(p.value),0));const bill=FinanceEngine.addBill(data,{name:`Fatura ${card.name} ${month}`,flow:'A_PAGAR',expected:total,dueDate:dueDate||invoiceDueDate(card,month),category:'Cartão',walletId:card.walletId});purchases.forEach(p=>{p.billId=bill.id;});return {bill,purchases,total};}
